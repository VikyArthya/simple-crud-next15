...
export const EditButton = ({ id }: { id: string }) => {
  return (
    <Link href={`/edit/${id}`} className="py-3 text-sm bg-gray-50 w-full rounded-bl-md hover:bg-gray-100 text-center">
      Edit
    </Link>
  );
};
...
export const DeleteButton = ({ id }: { id: string }) => {
  ...
  return (
    <form action={deleteImageWithId} className="py-3 text-sm bg-gray-50 rounded-br-md w-full hover:bg-gray-100 text-center">
      <DeleteBtn />
    </form>
  );
};
